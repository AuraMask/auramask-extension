...
/**
 ...
 composing them in a way to be exposed to the irmeta controller
 <br>- txStateManager
...
class TransactionController extends EventEmitter {
  ...
  async newUnapprovedTransaction(txParams, opts = {}) {
    log.debug(`IrMetaController newUnapprovedTransaction ${JSON.stringify(txParams)}`);
    ...
    return new Promise((resolve, reject) => {
      this.txStateManager.once(`${initialTxMeta.id}:finished`, (finishedTxMeta) => {
        switch (finishedTxMeta.status) {
          ...
          case 'rejected':
            return reject(cleanErrorStack(new Error('IrMeta Tx Signature: User denied transaction signature.')));
          case 'failed':
            return reject(cleanErrorStack(new Error(finishedTxMeta.err.message)));
          default:
            return reject(cleanErrorStack(new Error(`IrMeta Tx Signature: Unknown problem: ${JSON.stringify(finishedTxMeta.txParams)}`)));
        }
      });
    });
  }
  ...
  async addUnapprovedTransaction(txParams) {
    ...
    try {
      ...
      recipientBlacklistChecker.checkAccount(txMeta.irmetaNetworkId, normalizedTxParams.to);
...
//
  ...
  _updateMemstore() {
    ...
    const selectedAddressTxList = this.txStateManager.getFilteredTxList({
      ...
      irmetaNetworkId: this.getNetwork(),
    });
