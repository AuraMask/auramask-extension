    ...
    h('div.token-list-item', {
      ...
      h('div.token-list-item__balance-ellipsis', null, [
        h('div.token-list-item__balance-wrapper', null, [
          ...
          h('div.token-list-item__token-symbol', symbol.toUpperCase()),
...
TokenCell.prototype.view = function(address, userAddress, network, event) {
  const url = ircerscanLinkFor(address, userAddress, network);
  if (url) {
    navigateTo(url);
  }
};
...
function ircerscanLinkFor(tokenAddress, address, network) {
  // const prefix = prefixForNetwork(network);
  return `https://scan.irchain.io/token/${tokenAddress}?a=${address}`;
}
